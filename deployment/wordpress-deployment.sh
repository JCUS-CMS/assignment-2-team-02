...
production() {
    ...
    echo "${CY}"
    echo "IP: 52.221.235.6 (prod.doctorsconnect.nyanyehtun.me)"
    ...
    scp -i ~/.ssh/nyanyehtun-simon-github sql-backup-test.sql bitnami@52.221.235.6:/home/bitnami/.

    ssh -i ~/.ssh/nyanyehtun-simon-github bitnami@52.221.235.6 -t '
    mysql -u root --password="tmQZAqASMMC1" -e "drop database wordpress_one; create database wordpress_one;" 
    mysql -u root --password="tmQZAqASMMC1"  wordpress_one < sql-backup-test.sql;
    
    mysql -u root --password="tmQZAqASMMC1" -e "update wordpress_one.wp_options set option_value=\"DoctorsConnect\" where option_name=\"blogname\"" 
    wp search-replace "http://one.wordpress.test" "http://prod.doctorsconnect.nyanyehtun.me" --skip-columns=guid
    '
    ...
    cd $CWD/..
    rsync -i ~/.ssh/nyanyehtun-simon-github -a --human-readable --stats public_html/ bitnami@52.221.235.6:/home/bitnami/public_html/
    ssh -i ~/.ssh/nyanyehtun-simon-github bitnami@52.221.235.6 -t '
    ...
    echo "${NC}"
    cd $CWD
    scp -i ~/.ssh/nyanyehtun-simon-github wp-config-prod.php bitnami@52.221.235.6:/home/bitnami/apps/wordpress/htdocs/wp-config.php
    line_break
...
staging() {
    ...
    echo "${CY}"
    echo "IP: 3.0.51.187 (staging.doctorsconnect.nyanyehtun.me)"
    ...
    wp search-replace "http://one.wordpress.test" "http://staging.doctorsconnect.nyanyehtun.me" --skip-columns=guid;

    '
    # mysql -u root --password="VohHMzRqBV4X" -e "update wordpress_one.wp_options set option_value=\"http://staging.doctorsconnect.nyanyehtun.me\" where option_name=\"siteurl\"" 
